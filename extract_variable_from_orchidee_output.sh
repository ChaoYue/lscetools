...
##How to use:
# ~/extract_variable_from_orchidee_output.sh in_filename out_filename string_to_remove line_number
...
if [ ${line_number} -eq 1 ] ; then
  ...
  python /home/users/ychao/lscetools/remove_string_each_line.py tempfile.txt ${outfile} "${search_string_and_remove}"
  rm -fr tempfile*.txt
else
  grep "${search_string_and_remove}" -A $((line_number-1)) ${infile} > tempfile.txt
  python /home/users/ychao/lscetools/mergeline.py tempfile.txt tempfile1.txt $((line_number+1))  #line_number needs to be added 1 because after each pattern matching, 											           #grep command add another "--" afterwards.
  python /home/users/ychao/lscetools/remove_string_each_line.py tempfile1.txt tempfile2.txt "${search_string_and_remove}"
  python /home/users/ychao/lscetools/remove_string_each_line.py tempfile2.txt ${outfile} "--"  #we have to remove the "--" that's introduced when using grep command
  rm -fr tempfile*.txt
fi
